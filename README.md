<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0f0f,100:1a1a2e&height=120&section=header" width="100%" />

```
 ██╗  ██╗██╗██████╗ ████████╗ █████╗ ███╗   ██╗
 ██║ ██╔╝██║██╔══██╗╚══██╔══╝██╔══██╗████╗  ██║
 █████╔╝ ██║██████╔╝   ██║   ███████║██╔██╗ ██║
 ██╔═██╗ ██║██╔══██╗   ██║   ██╔══██║██║╚██╗██║
 ██║  ██╗██║██║  ██║   ██║   ██║  ██║██║ ╚████║
 ╚═╝  ╚═╝╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝
```

### CS @ Newton School of Science & Technology, Bangalore · Class of 2030

*Building AI systems that actually do things.*

[![Twitter/X](https://img.shields.io/badge/X-@ThatKJ-000000?style=flat-square&logo=x&logoColor=white)](https://twitter.com/ThatKJ)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-kirtan--joshi2412-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/kirtan-joshi2412)
[![GitHub](https://img.shields.io/badge/GitHub-ThatKJ-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/ThatKJ)

</div>

---

## About

First-year CS student who builds things at the intersection of **AI, automation, and the web**. I'm drawn to the gap between what AI *can* do and what's actually been built — and I spend most of my time closing that gap.

I care about products that feel designed rather than assembled. Clean systems over clever hacks. Tools that non-technical people actually want to use.

Currently targeting **GSoC 2027** — learning how large codebases work from the inside out.

---

## Projects

### [tokencalc](https://github.com/ThatKJ/tokencalc)
Compare token counts and API costs across 17 frontier AI models instantly. No API key required — 100% client-side. Built with Astro.

`Astro` `TypeScript` `OpenAI` `Anthropic` `Gemini` `Developer Tools`

---

### [cake-palace](https://github.com/ThatKJ/cake-palace)
Next-generation AI Bakery OS for a UAE client. Powers WhatsApp custom cake orders, CRM, and branch logistics for Cake Palace UAE.

`Next.js` `TypeScript` `Supabase` `WhatsApp Automation` `AI` `Client Work`

---

### [Portfolio](https://github.com/ThatKJ/Portfolio)
Personal portfolio built with React and Framer Motion. Clean UI with project showcases, skills, and contact section.

`React` `JavaScript` `Framer Motion` `Web Design`

---

### [AI-Powered-Writing-Assistant](https://github.com/ThatKJ/AI-Powered-Writing-Assistant)
An AI writing assistant built in HTML/JS.

`HTML` `JavaScript` `AI` `Writing Tools`

---

## Stack

**Languages** — TypeScript · JavaScript · Python · Java

**Frameworks** — Next.js · React · Astro

**AI & Automation** — Supabase · OpenAI-compatible APIs · Prompt Engineering · n8n

**Tools** — Git · Framer Motion · Tailwind CSS · Vercel

---

## Right Now

- First-year CS student, learning the fundamentals properly
- Working through a structured GSoC 2027 prep roadmap: Git workflow, Python depth, testing, navigating large open-source codebases
- Near-term goal: first merged PR to a real open-source project
- Designing a portfolio site — rejecting glassmorphism, chasing something genuinely distinctive

---

<div align="center">

*"Build systems that scale. Ship products that matter. Design like it's going somewhere."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,100:0f0f0f&height=80&section=footer" width="100%" />

</div>
